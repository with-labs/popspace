...
export function useParticipantState(participant: RemoteParticipant | LocalParticipant) {
  const [state, setState] = useState<ParticipantState>(participant.state as ParticipantState);

  useEffect(() => {
    const onStateChange = () => {
      setState(participant.state as ParticipantState);
    };
